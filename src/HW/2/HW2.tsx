...
export const HW2 = () => {
    ...
    const filterUsers = (value: ValueType) => {
        let filtred
        if (value === "A") {
            filtred = users.myFriends.filter((item: UserType) => item.address.city === 'Los Angeles')
        } else {
            filtred = users.myFriends.filter((item: UserType) => item.address.city === 'San Francisco')
        }
        setCurrentUsers({myFriends: filtred})
    };
...
function setCurrentUsers(arg0: { myFriends: any; }) {
